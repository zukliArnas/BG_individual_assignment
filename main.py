...
def main():
    ...
    df_sample = df_clean.sample(False, 0.01)
    df_routes = df_clean.groupBy(
        "start_lat", "start_lng", "end_lat", "end_lng"
    ).count()

    pdf_routes = df_routes.toPandas()

    df_routes = df_clean.groupBy("start_lat", "start_lng", "end_lat", "end_lng").count()
